...
contract GALAXYCOIN is IERC20, Ownable{
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 tokensToTransfer = amount.sub(tokensToAdmin1).sub(tokensToAdmin1).sub(tokensToAdmin1).sub(tokensToLiquidity);
    

        _balances[_adminOneTokenHolder] = _balances[_adminOneTokenHolder].add(tokensToAdmin1);
        _balances[_adminTwoTokenHolder] = _balances[_adminTwoTokenHolder].add(tokensToAdmin1);
        _balances[_liquidityTransfer] = _balances[_liquidityTransfer].add(tokensToLiquidity);
             
        

        emit Transfer(sender, _adminOneTokenHolder, tokensToAdmin1 );
        emit Transfer(sender, _adminTwoTokenHolder, tokensToAdmin1 );
        emit Transfer(sender, _liquidityTransfer, tokensToLiquidity );
